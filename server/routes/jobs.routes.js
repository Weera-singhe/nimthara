...
router.get("/job/:fileid/:jobindex", requiredLogged, async (req, res) => {
  try {
    ...
    return res.json({ thisJob, theseJobs, allPapers });
  } catch (err) {
    console.error("Error:", err.message);
    res.status(500).json({ success: false, message: err.message });
  }
...
router.post("/job/form2", requiredLogged, async (req, res) => {
  try {
    ...
    const estiStsBefore = beforeUpdate?.job_info?.esti_ok || false;
    const estiStsNow = job_info?.esti_ok || false;
    if (estiStsBefore !== estiStsNow) {
      if (!requiredLevel(req, res, "level_jobs", 3)) return;
    }
    ...
    if (tab === 0) {
      await pool.query(
        ` UPDATE job_jobs
        SET job_info = jsonb_set( jsonb_set( COALESCE(job_info, '{}'::jsonb), '{mate}', COALESCE($1::jsonb, '[]'::jsonb), true ), '{esti_ok}', to_jsonb(COALESCE($2::boolean, false)), true )
        WHERE jobfile_id = $3
          AND job_index  = $4 `,

        [
          JSON.stringify(job_info?.mate ?? []),
          job_info?.esti_ok ?? false,
          fileid,
          jobindex,
        ],
      );
    ...
    if (tab === 2 && jobStatusNow) {
      ...
      await pool.query(
        ...
        artwork = $3,
        job_status = $4,
        job_info = jsonb_set( jsonb_set(  jsonb_set( jsonb_set(COALESCE(job_info, '{}'::jsonb),
            '{start_at}',to_jsonb($5::text),true),
            '{others}',COALESCE($6::jsonb, '[]'::jsonb), true),
            '{finishing}',COALESCE($7::jsonb, '[]'::jsonb), true), 
            '{machine}',COALESCE($8::jsonb, '[]'::jsonb), true)
        WHERE jobfile_id = $9
          AND job_index  = $10
        `,
        [
          ...
          jobStatusNow,
          job_info.start_at,
          JSON.stringify(job_info?.others ?? []),
          JSON.stringify(job_info?.finishing ?? []),
          JSON.stringify(job_info?.machine ?? []),
          fileid,
          jobindex,
        ],
      );
    ...
    if (tab === 4 && jobStatusNow >= 2) {
      const safeLog = Array.isArray(delivery?.log) ? delivery.log : [];
      await pool.query(
        `UPDATE job_jobs
          SET 
            job_status = $1,
            delivery = jsonb_set( COALESCE(delivery, '{}'::jsonb), '{log}', $2::jsonb, true )
          WHERE jobfile_id = $3 AND job_index  = $4
          `,
        [jobStatusNow, JSON.stringify(safeLog), fileid, jobindex],
      );
    }
