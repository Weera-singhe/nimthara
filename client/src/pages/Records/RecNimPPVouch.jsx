...
import VouchPP from "../../forms/VouchPP";

export default function RecNimPPVouch({ user }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    axios
      .get(`${RECORDS_API_URL}/nim/vouchppr/${safeFormID}`)
      .then((res) => {
      ...
      <MyFormBox noBttn label="Job Tickets">
        <Autocomplete
          ...
          onChange={(_, j) => {
            if (!j) {
              setSelectedJob(null);
              navigate(`/records/nim/vouchppr`);
              return;
            }

            const newId = j.job_id;
            navigate(`/records/nim/vouchppr/${newId}`);
          }}
        ...
        <PrintViewer
          paperSize="A5"
          docName={`voucher_paper_#${jobfileTag(selectedJob?.jobfile_id)}_${selectedJob?.job_code || selectedJob?.job_index}`}
        >
          <VouchPP />
        </PrintViewer>
      </MyFormBox>
