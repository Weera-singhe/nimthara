...
import NimLogo from "../assests/LogoOneLineBlack.png";
import GtsLogo from "../assests/LogoOneLineBlack.png";

export default function VatInvoice({ data, company, items = [] }) {
  ...
  const compData = company === "gts" ? GtsData : NimData;
    ...
    >
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", gap: 1 }}>
        ...
        <Box sx={{ textAlign: "right", minWidth: 0 }}>
          <Box sx={{ mb: 1 }}>
            <img src={compData.logo} alt="logo" width={250} />
          </Box>
          <Typography sx={{ fontSize: 10, whiteSpace: "pre-line" }}>
            {compData.address}
          </Typography>
          <Typography sx={{ fontSize: 10 }}>Tel: {compData.phone}</Typography>
        </Box>
      </Box>
      ...
      >
        <InfoBlock
          ...
          lines={[
            data?.vendor?.name,
            data?.vendor?.address,
            data?.vendor?.phone ? `Tel: ${data.vendor.phone}` : null,
          ]}
        />
        <InfoBlock
          title="Ship To"
          lines={[
            data?.shipTo?.name ?? data?.company?.name,
            data?.shipTo?.address ?? data?.company?.address,
            data?.shipTo?.phone
              ? `Tel: ${data.shipTo.phone}`
              : data?.company?.phone
